...
public class ClienteIml implements ICliente {
	...
	public ClienteResponse BuscarClienteDocVen(BuscarNumerosResponse numeroCliente) {
		try {
			Mono<ClienteResponse> response = webClient.build().post().uri(URL + "/clienteVenta")
					.accept(MediaType.APPLICATION_JSON).body(Mono.just(numeroCliente), ClienteResponse.class).retrieve()
					.bodyToMono(ClienteResponse.class);
	...
	public ClienteResponseVenta buscarClienteDocumentoVenta(ClienteDocumentoVenta clienteDto) {
		try {
			Mono<ClienteResponseVenta> response = webClient.build().post().uri(URL + "/cliente/documento/Venta")
					.accept(MediaType.APPLICATION_JSON).body(Mono.just(clienteDto), ClienteResponseVenta.class).retrieve()
					.bodyToMono(ClienteResponseVenta.class);
	...
	public ClienteResponse buscarClienteDocumentoVenta(ClienteDocumento clienteDto) {
		try {
			Mono<ClienteResponse> response = webClient.build().post().uri(URL + "/clientes/documento")
					.accept(MediaType.APPLICATION_JSON).body(Mono.just(clienteDto), ClienteResponse.class).retrieve()
					.bodyToMono(ClienteResponse.class);
